...
import {
    getEthBalance, getTxCount, getZksEra, getZksLite, getZkSyncLastTX, getZkSyncBridge, exportToExcel
} from "@utils"
...
import {
    DeleteOutlined, DownloadOutlined, EditOutlined, PlusOutlined, SyncOutlined, UploadOutlined
    ...
    const handleOk = async () => {
        try {
            ...
            if (values.address.length !== 42) {
                notification.error({
                    message: "错误", description: "请输入正确的地址",
                }, 2);
                return;
            }
            ...
            if (index !== -1) {
                ...
                getZkSyncLastTX(values.address).then(({zkSyncLastTx}) => {
                    updatedData[index] = {
                        ...updatedData[index], zks2_last_tx: zkSyncLastTx,
                    };
                    setData(updatedData);
                    localStorage.setItem('addresses', JSON.stringify(data));
                ...
                getZksLite(values.address).then(({balance1, tx1}) => {
                    updatedData[index] = {
                        ...updatedData[index], zks1_balance: balance1, zks1_tx_amount: tx1,
                    };
                    setData(updatedData);
                    localStorage.setItem('addresses', JSON.stringify(data));
                ...
                getTxCount(values.address, "ethereum").then((eth_tx_amount) => {
                    updatedData[index] = {
                        ...updatedData[index], eth_tx_amount,
                    };
                    setData(updatedData);
                    localStorage.setItem('addresses', JSON.stringify(data));
                })
